...
export default function ResultPage() {
  ...
  const accents: Record<string, { button: string; text: string }> = {
    Calm: { button: 'bg-blue-500 hover:bg-blue-600', text: 'text-blue-600' },
    Happy: { button: 'bg-emerald-500 hover:bg-emerald-600', text: 'text-emerald-600' },
    Sad: { button: 'bg-slate-500 hover:bg-slate-600', text: 'text-slate-600' },
    Angry: { button: 'bg-rose-400 hover:bg-rose-500', text: 'text-rose-500' }
  }
        ...
        {stage !== 'done' ? (
          ...
          <div className="fade-in space-y-12">
            <section className="space-y-1">
              <p className={`text-lg font-medium ${accent.text}`}>{emotion} · Library · 92%</p>
            </section>
            ...
            <section className="mx-auto w-full max-w-xl space-y-5 rounded-2xl border border-slate-200 bg-white p-8 sm:p-10">
              <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
                <button className={`btn-polish w-full rounded-lg px-10 py-3 text-base font-semibold text-white sm:w-auto ${accent.button}`}>Generate New Sound</button>
